...
const T_WINDOW = 4; // seconds shown on the v–t plot
...
export default function ViscositySim() {
  ...
  if (params.current.eta !== eta || params.current.r !== r) {
    ...
    state.current = { y: 0, v: 0, t: 0, trace: [] };
  }
  ...
  const canvasRef = useCanvas((ctx, w, h, _t, dt) => {
    ...
    const termV = (2 * rm * rm * (rho - SIGMA) * g) / (9 * eta);
    // dv/dt = g' (1 − v/vt), g' = g(1 − σ/ρ) folded into vt = g'·τ
    const acc = g * (1 - SIGMA / rho) * (1 - s.v / Math.max(termV, 1e-9));
    s.v += acc * dt;
    s.t += dt;
    s.y += s.v * dt;
    s.trace.push({ t: s.t, v: s.v });
    while (s.trace.length > 2 && s.trace[1].t < s.t - T_WINDOW) s.trace.shift();
    if (s.t > T_WINDOW + 0.8) {
      s.y = 0;
      s.v = 0;
      s.t = 0;
    ...
    const frac = Math.min(1, (s.y / Math.max(termV * T_WINDOW, 1e-9)));
    ...
    const vFrac = Math.min(1, s.v / Math.max(termV, 1e-9));
    ...
    // velocity vector
    arrow(ctx, bx + rPx + 22, by, bx + rPx + 22, by + Math.max(6, vFrac * 40), SIM.sky, 2);
    label(ctx, `v = ${s.v.toFixed(3)} m/s`, bx + rPx + 28, by + vFrac * 40 + 12, SIM.sky, 10);
    ...
    label(ctx, "t (s) →", (px0 + px1) / 2, py1 + 12, SIM.dim, 9, "center");
    ...
    ctx.stroke();
    ctx.restore();
    label(ctx, `v_t = ${termV.toFixed(3)} m/s`, px1 - 6, vtY - 9, SIM.red, 10, "right");
    ...
    ctx.beginPath();
    s.trace.forEach((p, i) => {
      const xx = px0 + (p.t / T_WINDOW) * (px1 - px0);
      const vy = Math.min(1.1, p.v / Math.max(termV, 1e-9)) * 0.8; // 80% of plot at v_t
